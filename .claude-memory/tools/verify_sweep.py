# ruff: noqa
"""pass3 フルスイープの最終検証: 100%達成・除外run・衝突・ドレーン cap 到達の確認。"""
...
for p in sorted(glob.glob(f"{OUT}/raw/v2__*.csv")):
    ...
    if not rows:
        ...
        continue
    d = rows[0]
    e = envs.setdefault(env, {"n": 0, "req": 0, "comp": 0, "col": 0, "col_runs": 0, "cancel": 0, "worst_run": None})
    e["n"] += 1
    ...
    if tot not in (None, ""):
        ...
        e["comp"] += int(comp)
        if int(tot) != int(comp):
            fails.append((name, f"LC fail {int(tot) - int(comp)}/{tot}"))
...
print("=== env別サマリ ===")
grand_req = grand_comp = 0
for env, e in sorted(envs.items()):
    rate = f"{100 * e['comp'] / e['req']:.3f}%" if e["req"] else "-(母数0)"
    print(
        f"{env:14s} n={e['n']:3d} 必須LC {e['comp']}/{e['req']} = {rate}  "
        f"衝突計={e['col']:.0f} (発生run={e['col_runs']})  canceled計={e['cancel']}"
    )
    grand_req += e["req"]
    grand_comp += e["comp"]
print(f"\n総計: {grand_comp}/{grand_req} = {100 * grand_comp / grand_req:.4f}%" if grand_req else "")
print(f"\nLC失敗のある run: {len(fails)}")
for f in fails[:20]:
    print("  ", f)
print(f"\nドレーン cap(1500s) 到達 run: {len(drain_capped)}")
for n in drain_capped[:10]:
    print("  ", n)
